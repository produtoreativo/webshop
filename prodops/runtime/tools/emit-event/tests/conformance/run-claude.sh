...
SKILL_PATH="$REPO_ROOT/.claude/skills/prodops-emit-event/SKILL.md"
...
set +e
INVALID_OUT=$(echo '{"actor":{"player":"claude","agent":"t"}}' | bash "$TOOL" 2>/dev/null)
INVALID_EXIT=$?
set -e
...
FORBIDDEN_INPUT=$(jq -n --arg cid "$CORR" \
  '{"event":"Delivery.Bootstrap.Started","work-item-id":"76",
    ...
    "actor":{"player":"claude","agent":"t"},"payload":{}}')
set +e
...
INPUT_S=$(jq -n \
  ...
  '{"event":"Delivery.Bootstrap.Started","work-item-id":$wi,"iteration-id":$it,
    ...
    "actor":{"player":"claude","agent":"bootstrap-agent"},"payload":{}}')
set +e
