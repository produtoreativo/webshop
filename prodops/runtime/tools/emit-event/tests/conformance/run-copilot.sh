...
SKILL_PATH="$REPO_ROOT/.github/skills/prodops-emit-event/SKILL.md"
...
set +e
INVALID_OUT=$(echo '{"actor":{"player":"copilot","agent":"t"}}' | bash "$TOOL" 2>/dev/null)
INVALID_EXIT=$?
set -e
...
FORBIDDEN_INPUT=$(jq -n --arg cid "$CORR" \
  '{"event":"Delivery.Bootstrap.Started","work-item-id":"76",
    ...
    "actor":{"player":"copilot","agent":"t"},"payload":{}}')
set +e
...
INPUT_S=$(jq -n \
  ...
  '{"event":"Delivery.Bootstrap.Started","work-item-id":$wi,"iteration-id":$it,
    ...
    "actor":{"player":"copilot","agent":"bootstrap-agent"},"payload":{}}')
set +e
