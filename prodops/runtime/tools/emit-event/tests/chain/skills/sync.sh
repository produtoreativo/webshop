...
emit_event() {
  ...
  local input; input=$(jq -n --arg e "$event" --arg w "$WORK_ITEM_ID" --arg i "$ITERATION_ID" \
    ...
    '{"event":$e,"work-item-id":$w,"iteration-id":$i,"correlation-id":$c,"execution-id":$x,"actor":{"player":$p,"agent":"sync-agent"},"payload":{}}')
...
log "Emitting Sync.Started (correlation-id=$CORRELATION_ID)"
emit_event "Delivery.Sync.Started" "$EVIDENCE_DIR"
log "Sync work: rebase + align (simulated)..."
log "Sync work: branch rebased, artifacts aligned"
ok "Sync work: complete"
log "Emitting Sync.Completed (correlation-id=$CORRELATION_ID)"
emit_event "Delivery.Sync.Completed" "$EVIDENCE_DIR"
