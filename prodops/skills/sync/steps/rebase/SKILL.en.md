...
**Responsibility:** guarantee **repository integrity** — make the repository ready for merge by incorporating the latest base state into the feature branch without conflicts and without weakening tests. This is a **git** step, not a product step: it changes no behavior and aligns no ProdOps artifacts (that is `align`).
...
**Completion criterion — repository integrity:** complete when **all** of the following are true, simultaneously:
